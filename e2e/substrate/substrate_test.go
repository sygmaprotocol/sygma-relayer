...
func (s *IntegrationTestSuite) Test_Erc20Deposit_Substrate_to_EVM() {
	...
	err = s.substrateClient.TrackExtrinsic(context.Background(), extHash, sub)
	s.Nil(err)
...
func (s *IntegrationTestSuite) Test_Erc20Deposit_EVM_to_Substrate() {
	...
	_, err = bridgeContract1.Erc20Deposit(context.Background(), recipientMultilocation, amountToDeposit, s.evmConfig.Erc20LockReleaseResourceID, 3, nil, transactor.TransactOptions{
