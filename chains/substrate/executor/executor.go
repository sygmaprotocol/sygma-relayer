...
func (e *Executor) Execute(ctx context.Context, msgs []*message.Message) error {
	...
	ctx, span, logger := observability.CreateSpanAndLoggerFromContext(ctx, "sygma-relayer", "relayer.sygma.substrate.Execute")
	defer span.End()
...
func (e *Executor) watchExecution(ctx context.Context, cancelExecution context.CancelFunc, proposals []*chains.Proposal, sigChn chan interface{}, sessionID string) error {
	ctx, span, logger := observability.CreateSpanAndLoggerFromContext(ctx, "sygma-relayer", "relayer.sygma.substrate.watchExecution")
	defer span.End()
...
func (e *Executor) executeProposal(ctx context.Context, proposals []*chains.Proposal, signatureData *common.SignatureData) (types.Hash, *author.ExtrinsicStatusSubscription, error) {
	_, span, _ := observability.CreateSpanAndLoggerFromContext(ctx, "sygma-relayer", "relayer.sygma.substrate.executeProposal")
	defer span.End()
	sig := []byte{}
	...
	if err != nil {
		return types.Hash{}, nil, observability.LogAndRecordErrorWithStatus(nil, span, err, "failed to ExecuteProposals")
	}
	return hash, sub, err
}
